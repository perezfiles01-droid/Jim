...
| 21 | IsEdrmsCompliant | Yes / No | Whether the parent site is an EDRMS compliant site | true, false | true | **AvePoint Cloud Governance** | **SOURCE FOUND 14 Aug 2026.** Workspace report, column `EDRMS Site Type`, populated on 1,032 of 1,209 workspaces in the test tenant. Covers CG created and CG adopted sites alike |
...
| 21 | IsEdrmsCompliant | **Records Management:** decides which documents count toward Total Documents in EDRMS Compliant Sites, 3.47M. Defines the population of the entire table | **Missing. Gap 3** | No such column and no rule anywhere in the workbook or the database | **ANSWERED 14 Aug 2026: the third option was right.** AvePoint Cloud Governance, Directory, Workspace report, column `EDRMS Site Type`. Populated on 1,032 of 1,209 workspaces in the test tenant with the single value `EDRMS Project Site`. It covers both CG created and CG adopted sites, which matters because Leah Bancale confirmed EDRMS sites exist that did not originate in Cloud Governance and are converted afterwards. Still to do: get the PRODUCTION export, and validate the register against the installed app once on a sample |
