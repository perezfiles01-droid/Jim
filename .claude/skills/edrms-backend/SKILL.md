---
name: edrms-backend
description: "Act as the PostgreSQL and data sourcing expert for the ADB EDRMS Utilization Report. Use this skill whenever the question is where a figure comes from, whether a metric can be produced, what SQL or Microsoft Graph call would produce it, which of the four reporting tables and which column carries it, or whether something is buildable, sourceable, testable or blocked. Use it to design or review a table, column, key, join, refresh job, snapshot or retention policy, to write or check the query behind a KPI, to judge whether a requirement needs a new column, a new join, an application change or a new data source, and for questions about the drm-npr database, the Records table, the Microsoft 365 usage reports, the SharePoint term store, Purview retention labels or the AvePoint EDRMS application. Use it even for a quick yes or no, because the recurring failure here is a confident wrong answer from an assumed column name or API window, and this skill forces the check first."
---
