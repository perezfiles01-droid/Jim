DASHBOARDS.dd=(function(){
  ...
  const TRACE=[
    ...
    ["Records Management","Records Declared This Month","Utilization Report Table","Count the declared rows where CreatedDate falls in the current month","ok"],
    ["Records Management","Records declared by month","Utilization Report Table","The same count grouped by month of CreatedDate","ok"],
    ["Records Management","Records declared by year","Utilization Report Table","The same count grouped by year of CreatedDate","ok"],
    ["Sites and Libraries","Libraries with no declared records","Site Activity Table and Utilization Report Table","Libraries counted from LibraryCount, minus the distinct ListId values present in the report table. <b>Not a scan job</b>, and the join is safe because ListId is verified to match SharePoint's own list GUID","ok"],
    ["Retention","Records due within 30 and 90 days","Utilization Report Table","The same count as the 12 month figure at nearer horizons. The three necessarily nest","ok"],
    ["Retention","Records with a retention schedule","Utilization Report Table","Count the declared rows where EDRMSRetentionLabel is not null","ok"],
    ["Retention","Records without a retention schedule","Utilization Report Table","The same count where EDRMSRetentionLabel is null. These have no due date, so they sit outside every disposal figure","ok"],
    ["Retention","Records beyond retention period","Utilization Report Table","Count the declared rows where EDRMSDueDateForDisposal is already past. <b>Does not mean still awaiting action</b>, since nothing records whether a disposal happened","ok"],
